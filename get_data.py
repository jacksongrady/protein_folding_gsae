...
for entry in tqdm(range_to_filename.keys()):
    ...
    constructors = {
        ...
        "pdb_dir": f"/data/lab/de_shaw/all_trajectory_slices/GB3/{entry}",
    ...
    for idx, pdb in enumerate(tqdm(pdbs)):
        path = f"/data/lab/de_shaw/all_trajectory_slices/GB3/{entry}" + '/' + pdb
        try:
            graph_list.append(
    ...
    #         print(i)
    #         pyg_list.remove(i)
    with open(f"graphs/{range_to_graphname[entry]}.pkl", "wb") as file:
...
for i, entry in enumerate(tqdm(arr)):
    ...
    if split2 == 'pkl' and split1 != 'total_graphs':
        ...
        with open("./graphs/" + entry, "rb") as file:
            graphs = pickle.load(file)
            for graph in graphs:
                total_graphs.append(graph)

with open("./graphs/total_graphs.pkl", 'wb') as out:
